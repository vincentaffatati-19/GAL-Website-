...
export async function renderProgressSurface(): Promise<string> {
  try {
    ...
    return `<section class="equipment-list">${progress.map((row)=>`<article class="equipment-card"><p class="eyebrow">${insightLabel(row.status) ?? ''}</p><h2>${row.headline}</h2><p>${row.golfer_message}</p></article>`).join('')}</section>`;
  } catch {
