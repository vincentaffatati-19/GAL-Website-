...
export async function renderDriverFit(evidence: DriverEvidenceInput[] = []): Promise<string> {
  try {
    ...
    const targetSection = model.missingEvidence
      ...
      : `<section><h2>What GAL is fitting for</h2>${model.targetProfile.characteristics.map((target) => `<article><h3>${target.key}</h3><p>${target.direction} — ${target.rationale}</p><small>Evidence: ${target.evidenceState}</small></article>`).join('')}</section>`;
    const candidateSection = model.missingEvidence
      ...
      : `<section><h2>Candidate configurations</h2>${model.candidates.length ? model.candidates.map((candidate) => `<article><h3>${candidate.item.familyName}</h3><p>${candidate.configuration.name}</p><p>${candidate.configuration.readiness === 'AI_FIT_LIMITED' ? `Limited evidence — ${candidate.configuration.blockingGapCount} blocking gap(s)` : 'AI Fit ready'}</p></article>`).join('') : '<p>No governed Driver configurations currently qualify.</p>'}</section>`;
