...
const UX10_KEYS: Record<EquipmentCategory, Ux10BagCategory['key']> = {
  DRIVER: 'driver', FAIRWAY_WOOD: 'fairway', HYBRID: 'hybrid', IRON: 'irons', WEDGE: 'wedges', PUTTER: 'putter', GOLF_BALL: 'ball',
};

function bagCategory(category: EquipmentCategory, item?: BagEquipmentView): Ux10BagCategory {
  const label = CATEGORY_LABELS[category];
  if (!item) {
    return {
      key: UX10_KEYS[category],
      label,
...
function equipmentDetail(item: BagEquipmentView): string {
  const config = item.state === 'KNOWN'
    ...
    : 'Configuration details needed';
  return `<article class="bag-detail-card ux10-panel"><p class="eyebrow">${escapeHtml(CATEGORY_LABELS[item.category])}</p><h3>${escapeHtml(item.equipmentName)}</h3><p>${config}</p><p class="ux10-fit-caution">Equipment identity alone does not prove fit.</p>${item.fittingHref ? `<a class="button" href="${escapeHtml(item.fittingHref)}">${item.state === 'KNOWN' ? 'Open Driver Fit' : 'Complete Driver Details'}</a>` : ''}</article>`;
}
...
export async function renderMyBag(): Promise<string> {
  try {
    ...
    const environment = renderUx10BagEnvironment({ categories, contextLabel: 'My Bag equipment environment' });

    return `<section class="my-bag-experience ux10-my-bag">
      <header class="my-bag-heading ux10-section-heading"><div><p class="eyebrow">My Bag</p><h2>Your equipment, as GAL currently knows it.</h2><p>Select a category to see what is known, what still needs details, and where governed equipment intelligence can help. Missing information is not treated as a good fit.</p></div><a class="bag-guide-link" href="#bag-guide">How My Bag Works</a></header>
      <div class="ux10-bag-environment-slot">${environment}</div>
      <section class="bag-detail-grid ux10-bag-details" id="bag-details" aria-label="Recorded equipment details">
        ${items.length ? items.map(equipmentDetail).join('') : '<article class="bag-detail-card bag-empty ux10-panel"><h3>Build your bag</h3><p>Add equipment so GAL can connect your bag to governed equipment intelligence.</p></article>'}
      </section>
      <section class="my-gal-state ux10-panel" id="customize"><p class="eyebrow">Customize My Bag</p><h2>Presentation preferences stay separate from equipment truth.</h2><p>Bag appearance can be personalized independently from your tee-box background without changing what GAL knows about your equipment or fit.</p></section>
      <section class="my-gal-state ux10-panel" id="bag-guide"><p class="eyebrow">Bag User’s Guide</p><h2>How My Bag Works</h2><p>Known equipment and configuration details come from your bag and governed GAL Equipment Knowledge. “Not evaluated” or missing configuration means GAL needs more evidence; it does not mean the club is optimized. Driver fitting uses the same equipment truth and shows characteristics before brands.</p></section>
    </section>`;
  } catch (error) {
