  ...
  try {
    ...
    return `<section class="equipment-list" aria-label="My Bag equipment">${items.map((item) => `
      <article class="equipment-card">
        <p class="eyebrow">${escapeHtml(item.category.replaceAll('_',' '))}</p>
        <h2>${escapeHtml(item.equipmentName)}</h2>
        <p>${item.state === 'KNOWN' ? (item.configuration ? `Configuration: ${escapeHtml(item.configuration.name)}` : 'Configuration linked to GAL Equipment Knowledge.') : 'Configuration details needed'}</p>
        ${item.fittingHref ? `<a class="button" href="${escapeHtml(item.fittingHref)}">${item.state === 'KNOWN' ? 'Open Driver Fit' : 'Complete Driver Details'}</a>` : ''}
