-- GAL Equipment Knowledge Wave A: schema parity baseline
-- Additive/non-destructive. Reproduces required production legacy/reference structures in staging.

-- Guarded enums used by the retained legacy/reference tables.
do $$ begin
...
create table if not exists public.gal_bags (
  ...
  bag_id text not null unique default gal_public_id('GAL-BAG'),
...
create table if not exists public.gal_decision_snapshots (
  id uuid primary key default gen_random_uuid(),
  decision_snapshot_id text not null unique default gal_public_id('GAL-DS'),
  user_id uuid not null references public.gal_users(id) on delete cascade,
...
create table if not exists public.gal_bag_items (
  id uuid primary key default gen_random_uuid(),
  bag_item_id text not null unique default gal_public_id('GAL-BI'),
...
create table if not exists public.gal_profile_facts (
  ...
  confidence numeric not null default 1.000 check (confidence between 0 and 1),
...
create table if not exists public.gal_valuation_snapshots (
  id uuid primary key default gen_random_uuid(),
  valuation_snapshot_id text not null unique default gal_public_id('GAL-VS'),
  user_id uuid not null references public.gal_users(id) on delete cascade,
...
create index if not exists gal_active_slot_idx on public.gal_bag_items(bag_id,slot_code) where item_type='CLUB' and bag_status='IN_BAG';
...
-- Explicit deny for internal legacy/reference tables. Service role bypasses RLS.
