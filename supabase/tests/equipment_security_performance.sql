...
do $$ begin
  if not has_table_privilege('anon','public.gal_equipment_guide_v','SELECT') then raise exception 'anon guide view select missing'; end if;
  if has_table_privilege('anon','public.gal_equipment_ai_fit_v','SELECT') then raise exception 'anon must not read AI Fit view'; end if;
end $$;
