...
6. Longitudinal mutations must pass through a server-side trusted boundary that invokes existing service-only RPCs.
...
Create a dedicated authenticated portal application mounted under a stable GAL route such as `/portal/` or a dedicated application subdomain.
...
Create server-side endpoints or Supabase Edge Functions for every browser-initiated longitudinal mutation.

The trusted layer may hold the service-role key and call only the governed RPCs required for the requested user action.
...
The server must resolve the authenticated GAL user from the access token/session and must not trust a browser-supplied `user_id` as authority.
...
Portal server functions must derive the GAL user from the authenticated Supabase identity.
...
Prefer narrow action-specific endpoints rather than a generic RPC passthrough.

Initial interface:

- `POST /api/insights/:insightId/present`
- `POST /api/insights/:insightId/acknowledge`
- `POST /api/insights/:insightId/dismiss`
- `POST /api/insights/:insightId/snooze`
- `POST /api/insights/:insightId/action`

Future application-owned endpoints may record outcomes as business workflows complete.

Each endpoint returns a small normalized result:
...
4. Portal assembles a presentation model client-side or through a read-only server view/API if later needed.
5. When an insight is actually presented according to product rules, portal calls the trusted presentation endpoint.
6. Server resolves the golfer and invokes `gal_record_insight_presentation`.
7. UI reflects any cooldown or terminal delivery state returned by the server.

### 9.2 Golfer response

1. User selects an allowed response.
2. Browser sends insight ID plus action-specific fields only.
3. Server validates session, user ownership, and action.
4. Server calls `gal_record_insight_response`.
...
`Take Action -> buyer/recommendation workflow -> qualifying event -> server records outcome -> governed resolution logic may later resolve or regress insight.`
...
The portal may display optional analytics consent controls later, but any withdrawal flow must append a new consent record rather than mutate history and must trigger the governed learning reconciliation path server-side.
...
- endpoint trusts browser-supplied `user_id` for authorization;
- one authenticated golfer can read or mutate another golfer's insight data;
- subscriber entitlement logic bypasses RLS;
- generic server endpoint forwards arbitrary RPC/table requests.
...
- Server endpoint can mutate only the authenticated user's valid target insight.
...
6. trusted server mutation boundary;
7. shared entitlement framework with base registered-user and subscriber states;
...
- a subscriber uses the same portal components and contract with entitlement-driven enhancements;
- golfer responses are recorded only through the trusted server boundary;
